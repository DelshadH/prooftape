...
9. downloads registry bytes, verifies SHA-256, `dist.integrity`, `dist.shasum`,
   ...
   commit, `alpha` dist-tag, and any initial registry-assigned `latest` tag
   only when it points to the sole reviewed version, with an absolute five-minute
   propagation deadline that includes registry request time, then runs
